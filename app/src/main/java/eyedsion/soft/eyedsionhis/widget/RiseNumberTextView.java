    ...
    private void runFloat() {
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                if (flags) {
                    setText(Utils.format(",##0.0").format(Double.parseDouble(valueAnimator.getAnimatedValue().toString())) + "");
                    if (valueAnimator.getAnimatedValue().toString().equalsIgnoreCase(number + "")) {
                        setText(Utils.format(",##0.0").format(Double.parseDouble(number + ""))+Unit);
                    }
                } else {
                    setText(Utils.format("##0.00").format(Double.parseDouble(valueAnimator.getAnimatedValue().toString())) + "");
                    if (valueAnimator.getAnimatedValue().toString().equalsIgnoreCase(number + "")) {
                        setText(Utils.format("##0.00").format(Double.parseDouble(number + ""))+Unit);
                    }
                }
    ...
    private void runInt() {
        ...
        valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator valueAnimator) {
                ...
                setText(valueAnimator.getAnimatedValue().toString()+Unit);
                if (valueAnimator.getAnimatedFraction() >= 1) {
                    mPlayingState = STOPPED;
                    if (mEndListener != null)
                        mEndListener.onEndFinish();
